...
private class SearchScopePopupContent(scopes: List<ScopeItem>) {
    ...
    init {
        ...
        allScopes =
            scopes.sortedWith(Comparator.comparing({ it.scope.title }, NaturalComparator.INSTANCE)).map { scope ->
                SearchScopeItem.Element(
                    scope.scope, propertyGraph.property(scope.isSelected)
                )
            }
    ...
    init {
        val items = ContainerUtil.concat(
            getItems(standardGroup, standardScopes), getItems(customGroup, customScopes)
        )
        ...
        list.whenMousePressed {
            when (val scope = list.selectedValue) {
                is SearchScopeItem.Group -> scope.property.set(
                    ThreeStateCheckBox.nextState(
                        scope.property.get(), false
                    )
                )
    ...
    companion object {
        ...
        private fun getItems(
            group: SearchScopeItem.Group, scopes: List<SearchScopeItem.Element>
        ): List<SearchScopeItem> {
            if (scopes.isEmpty()) {
                return emptyList()
            }
...
private class SearchScopeRenderer : ListCellRenderer<SearchScopeItem> {
    ...
    override fun getListCellRendererComponent(
        ...
        val checkBox = when (value) {
            is SearchScopeItem.Group -> ThreeStateCheckBox(value.title).apply { isThirdStateEnabled = false }
                .bind(value.property)

            is SearchScopeItem.Element -> JCheckBox(value.title).bind(value.property)
        }
        val indent = when (value) {
            is SearchScopeItem.Group -> 0
            is SearchScopeItem.Element -> 1
        }
        return checkBox.apply { border = emptyListCellBorder(list, index, indent) }
...
private class SearchScopeDropDownLink(
    ...
    property.get(), { createPopup(property.get(), it::selectedItem.setter) }) {
    ...
    override fun itemToString(item: List<ScopeItem>): @NlsSafe String {
        ...
        return when {
            selectedScopes.isEmpty() -> ExternalSystemBundle.message("external.system.dependency.analyzer.scope.none")

            selectedScopes.size == item.size -> ExternalSystemBundle.message("external.system.dependency.analyzer.scope.any")

            selectedScopes.size == standardScopes.size && selectedScopes.size == selectedStandardScopes.size -> ExternalSystemBundle.message(
                "external.system.dependency.analyzer.scope.standard"
            )

            selectedScopes.size == customScopes.size && selectedScopes.size == selectedCustomScopes.size -> ExternalSystemBundle.message(
                "external.system.dependency.analyzer.scope.custom"
            )

            else -> StringUtil.shortenPathWithEllipsis(
                NlsMessages.formatNarrowAndList(selectedScopes.map { it.scope.title }), 30, true
            )
        }
    }
